...
sQ = sqrt(Q);  % = lambda *L
lambda = sQ/L;
lambda_g = 0.09;  %the space constant
lambda_g = 0.1;
...
phi = sinh(lambda*x)'*a+cosh(lambda*x)'*b;
phi_i = rc*phi/(re+rc) + (rc*J*lambda*x'*ones(1,N+1))/(re+rc) + ones(nx+1,1)*gamma;
phi_e = -re*phi/(re+rc) + (rc*J*lambda*x'*ones(1,N+1))/(re+rc) + ones(nx+1,1)*gamma;
dphi = lambda*cosh(lambda*x)'*a+lambda*sinh(lambda*x)'*b;
dphi_i = rc*dphi/(re+rc) + (rc*J*lambda)/(re+rc);
dphi_e = -re*dphi/(re+rc) + (rc*J*lambda)/(re+rc);
