...
%  
function HH_wave
global gnabar gkbar Vna Vk Vl  Veq Tfact N dg sc gl taum Cm 
...
Gm = gnabar*m^3*h + gkbar*n^4 + gl;
Rm = 1/Gm;
taum = Cm*Rm;  % the time constant  in ms
%taum = 0.434;  % this is a fudged value of taum
...
function currents = IV(s)
    global gnabar gkbar Vna Vk Vl Veq N  gl
    V = s(1:N);
    m = s(N+1:2*N);
    h = s(2*N+1:3*N); 
    n = s(3*N+1:4*N);
