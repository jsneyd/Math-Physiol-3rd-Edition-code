...
for i=1:n
    p.T = period(i);
    p.w = 1;  % hold w fixed
    p.h = stimweight*p.T/p.w;  % vary h
    options = odeset('RelTol',1e-5,'AbsTol',1e-5,'MaxStep',0.1);
    tspan = linspace(0,10*p.T,1000); % long enough to sit on the periodic solution
