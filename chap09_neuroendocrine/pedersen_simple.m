...
getparams
init = [0,0,0,0];  % to get a response at the first step, we need some Y to start out
...
init=[0,0,0,4];
G = [0, 200,0,200];     % glucose levels  
times = [0   10 60 65];     % times of step application
tspan = linspace(0,750,5000);
[T,sol] = ode15s(@(t,x)rhs(t,x,G,times),tspan,init);
X = sol(:,1);
F = sol(:,2);
I = sol(:,3);
...
p.pminus = 0.11;
p.r = 0.0023;
% these are not needed for this code% p.ng = 600;  % Number of values of g
